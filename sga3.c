...
int       select(int popsize, double sumfitness, ind_t *old);
...
/* nvar-------------total number of design variables
   xvar[]-----------vector of the design variables
   rvar[]-----------interpolation ranges for design variables
   max_ult----------stores the maximum occurance of a raw fitness
*/
...
initialize(&popsize,&lchrom,&maxgen,&pcross,&pmutation,oldpop,
           &max,&min,&avg,&sumfitness,&nmutation,&ncross,&nvar,
           rvar,&max_ult,gen);
...
generation(popsize,&sumfitness,oldpop,newpop,lchrom,&ncross,&nmutation,
           pcross,pmutation,nvar,rvar,avg);
...
int select(int popsize, double sumfitness, ind_t *pop)
{
int j;
